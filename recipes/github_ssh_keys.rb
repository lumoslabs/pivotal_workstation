...
execute "create SSH key pair for Github" do
  command "ssh-keygen -N '' -f #{WS_HOME}/.ssh/github_dsa"
  user WS_USER
  not_if "test -e #{WS_HOME}/.ssh/github_dsa"
end

execute "add Github configuration to .ssh/config" do
  config = "\n\nHost github.com\n  User git\n  IdentityFile #{WS_HOME}/.ssh/github_dsa"
  command "echo '#{config}' >> #{WS_HOME}/.ssh/config"
  not_if "grep 'github_dsa' #{WS_HOME}/.ssh/config"
  user WS_USER
end
...
if node["github_username"].to_s != "" && node["github_password"].to_s != ""
  ...
  execute "upload ssh key to github if it does not already exist there" do
    not_if <<-SH
      curl #{curl_options} https://api.github.com/user/keys | grep "`cat #{WS_HOME}/.ssh/github_dsa.pub` | cut -f 2 -d ' '`"
    SH
    command <<-SH
    curl -X POST --verbose #{curl_options} \
         --data "{ \\"key\\": \\"`cat #{WS_HOME}/.ssh/github_dsa.pub`\\", \\"title\\": \\"#{github_name}\\" }" \
